...
var input = `1640	590	93	958	73	1263	1405	1363	737	712	1501	390	68	1554	959	79
...
func part1() int {
	...
	for _, row := range rows {
		...
		intCells := make([]int, len(cells))
		for i, cell := range cells {
			intCells[i], _ = strconv.Atoi(cell)
		}
		sort.Ints(intCells)
		total += int(math.Abs(float64(intCells[0] - intCells[len(intCells)-1])))
	}
...
func part2() int {
	...
	for _, row := range rows {
		...
		sort.Sort(sort.Reverse(sort.IntSlice(intCells)))
	outer:
		for index, i := range intCells[:len(intCells)-1] {
			for _, j := range intCells[index+1:] {
				if i%j == 0 {
					total += int(i / j)
					break outer
				}
